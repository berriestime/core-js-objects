...
function makeWord(lettersObject) {
  ...
  const wordLength = allPositions.length ? Math.max(...allPositions) + 1 : 0;

  const wordArray = new Array(wordLength).fill('');

  Object.entries(lettersObject).forEach(([letter, letterPositions]) => {
...
class Selector {
  ...
  combine(selector1, combinator, selector2) {
    ...
    this.combinatorSelectors = [selector1, selector2];
    return this;
  }
  ...
  stringify() {
    ...
    if (this.combinatorSelectors.length > 0) {
      return `${this.combinatorSelectors[0].stringify()} ${
        this.combinator
      } ${this.combinatorSelectors[1].stringify()}`;
    }
